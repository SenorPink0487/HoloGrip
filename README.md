...
本项目不仅旨在通过直观的三维可视化手段降低理工科学习的认知门槛，更深度集成了 **Google Gemini 多模态大模型**，支持将 2D 立体几何题目截图一键重构为可交互的 3D 空间模型，打破传统教辅的维度限制。
...
### 3. 🤖 AI 几何识图 (Gemini 多模态驱动)
- **多模态 3D 重构**：支持上传或裁剪立体几何题目的 2D 截图，调用 Gemini 3.5 多模态大模型自动进行空间测算与拓扑推导，并在三维空间中还原出带顶点标签的精确 3D 几何模型，直接置于全息几何黑板上进行旋转与拆解交互。
