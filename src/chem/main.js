...
function applyMolecule(mol, product, comp) {
  ...
  // 用 PubChem 返回的分子式回填成分，保证圆环/图例也能明确显示
  ...
  const isMix = product?.kind === 'mixture' || product?.kind === 'blend'
  ...
  if (el.hudKicker) {
    el.hudKicker.textContent =
      product?.kind === 'blend' ? '加法混合' : product?.kind === 'mixture' ? '混合物' : '单质/化合物'
  }
...
function updateOperandIndices() {
  ...
  if (el.searchLabelText) {
    el.searchLabelText.textContent = isBlend
      ? `加法混合：${fields.length} 种物质按质量比合并`
      : '输入物品、化学式或 SMILES'
  }

  if (el.searchHint) {
    el.searchHint.innerHTML = isBlend
      ? '多项横向混合 · 可写 <code>水:2</code> 调比例 · 点 <strong>+</strong> 加项 · 点击输入框上的 <strong>×</strong> 删除'
      : '化学表达式完整显示 · 点 <strong>+</strong> 可添加多种物质进行混合'
  }
}
...
async function search() {
  ...
  setStatus('loading', parts ? '加法混合中…' : '解析中…')

  setOverlay({
    hidden: false,
    loading: true,
    error: false,
    kicker: parts ? '加法混合' : '解析中',
    title: parts ? '正在把两种东西加在一起…' : '正在拆解成分与结构…',
  ...
  try {
    ...
    const comp = components[activeIndex]
    applyMolecule(result.molecule, session.product, comp)
    // applyMolecule 可能回填了 formula，再刷一次圆环
    paintComposition()
    setOverlay({ hidden: true, loading: false })
    setStatus(
      'ready',
