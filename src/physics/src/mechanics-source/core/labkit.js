...
export function addSimpleGround(engine, { size = 40, color = 0x121a2c } = {}) {
  ...
  return { floor, floorBody: body, surfaceY: 0 };
}
...
export function addLabBench(engine, {
  ...
  const body = new CANNON.Body({
    ...
    position: new CANNON.Vec3(px, height + 0.03, pz),
  });
  engine.world.addBody(body);
  engine.bodies.push(body);

  return { group: g, surfaceY: height + 0.03, body, width, depth };
}
...
export function createPhysicsSphere(engine, {
  ...
  visual.userData.body = body;
  engine.scene.add(visual);
  engine.meshes.push(visual);

  return { mesh: visual, body, radius };
}
