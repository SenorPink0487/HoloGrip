...
function gaussSpecs(d) {
  // Position is 3D-drag only — no x/y/z desk sliders.
  const charges = Array.isArray(d?.charges) ? d.charges : [];
  const selected = charges.find((c) => c.id === d?.selectedId) || null;
  const out = [
    range('radius', 'R/m', 1.2, 4.2, { setAction: 'gauss-set' }),
  ];
  if (selected) {
    out.push(
      range('q', '|Q|/μC', 0.2, 3, { digits: 1, setAction: 'gauss-set' }),
    );
  }
  return out;
}
...
export function getDeskSliderConfig(stationId, expId, data = {}, experiment = null) {
  ...
  if (stationId === 'electro') {
    if (expId === 'faraday_induction') {
      // Only live-effect knobs: B and x immediately move the field / rod and
      // drive induced/motional current while dragged. Targets & Δt stay on the
      // content screen for preset-then-play.
      return {
        title: '法拉第 · 实时调节',
        specs: [
          range('B', '磁场 B', -3, 3, { unit: 'T', setAction: 'faraday-set' }),
          range('x', '棒位置 x', 1.2, 8, { setAction: 'faraday-set' }),
        ],
      };
    }
    ...
    if (expId === 'gauss_theorem') {
      return { title: '高斯定理 · 参数', specs: gaussSpecs(d) };
    }
