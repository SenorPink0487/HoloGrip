...
class SourceEngineAdapter {
  ...
  step(dt, tick) {
    ...
    while (this.accumulator >= this.fixedTimeStep && steps < maxSubSteps) {
      this.onPreStep?.(this.fixedTimeStep, this.simTime);
      this.world.step(this.fixedTimeStep);
      this.simTime += this.fixedTimeStep;
      this.accumulator -= this.fixedTimeStep;
      steps += 1;
    }
    if (this.accumulator > this.fixedTimeStep * 2) {
      this.accumulator = 0;
    }
    this.syncMeshes();
    tick?.(this.fixedTimeStep, this.simTime);
  }
...
export class MechanicsSourceRuntime {
  ...
  disposeBuild() {
    ...
    if (this.engine) {
      for (const body of this.engine.bodies) this.engine.world.removeBody(body);
    }
    this.content = null;
    this.engine = null;
    this.ui = null;
