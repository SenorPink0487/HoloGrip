...
/** Full-width action chip texture for discrete desk controls (e.g. 记录当前读数). */
function makeActionButtonTexture(label, accent = '#ec4899') {
  ...
  const r = Math.round(c.height * 0.28);
  // Soft accent fill
  ...
  ctx.textBaseline = 'middle';
  ctx.font = `bold ${Math.round(c.height * 0.42)}px "Microsoft YaHei", "Segoe UI", sans-serif`;
  ctx.fillText(String(label || ''), c.width / 2, c.height * 0.52);
...
export function createDeskSliderPanel({
  ...
  function applySlotVisual(slot) {
    ...
    const isAction = spec.kind === 'action';
    slot._isAction = isAction;
    if (slot.actionPlane) slot.actionPlane.visible = isAction;
    setRangePartsVisible(slot, !isAction);

    if (isAction) {
      const labelText = spec.label || '操作';
      const headerKey = `action|${labelText}|${spec.action || ''}`;
      if (slot._headerKey !== headerKey) {
        slot._headerKey = headerKey;
        const { tex, meshW, meshH } = makeActionButtonTexture(labelText, accentHex);
  ...
  function setSpecs(specs = [], _title = '参数调节') {
    ...
    const sig = list.map((s) => `${s.kind || 'range'}|${s.action}|${s.setAction}|${s.key}|${s.target}|${s.label}|${s.min}|${s.max}`).join(';');
  ...
  function syncValues(getValue) {
    ...
    for (let i = 0; i < activeCount; i += 1) {
      const slot = slots[i];
      if (!slot.spec || slot.spec.kind === 'action') continue;
  ...
  function pickFromRay(raycaster) {
    ...
    if (spec.kind === 'action') {
      return {
        id: `desk-${stationId}-${spec.action || spec.key || 'action'}`,
        role: 'desk_action',
        action: spec.action,
        key: spec.key ?? null,
        kind: 'action',
