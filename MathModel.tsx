...
export function MathModel() {
          ...
          for (let j = i + 1; j < allLineSegments.length; j++) {
            ...
            if (Math.abs(denom) > 1e-8) {
              ...
              if (cp1.distanceTo(cp2) < 0.05) {
                checkSnapPoint(cp1.clone().add(cp2).multiplyScalar(0.5));
              }
            }
          }
        }
  ...
  const presetEdgeLabels = useMemo(() => {
    ...
    return edges.map(([i, j]) => {
      ...
      return { p1, p2, key: `${v1.label}-${v2.label}` };
    });
  }, [activeModel, showAllLengths]);
  ...
  const customEdgeLabels = useMemo(() => {
    ...
    return activeCustomModel.edges.map((edge, idx) => {
      ...
      const p1 = new THREE.Vector3(v1.x, v1.y, v1.z);
      const p2 = new THREE.Vector3(v2.x, v2.y, v2.z);
      return { p1, p2, key: `custom-edge-${idx}` };
    }).filter(Boolean) as { p1: THREE.Vector3; p2: THREE.Vector3; key: string }[];
  }, [activeCustomModel, showAllLengths]);
    ...
    <group ref={groupRef} position={[0, 0, -2]}>
      {hasActiveModel && (
        <group>
          ...
          {/* ========== 棱边长度自动标注 ========== */}
          {presetEdgeLabels.map(({ p1, p2, key }) => (
            <LineLengthLabel
              key={key}
              p1={p1}
              p2={p2}
          ...
          {customEdgeLabels.map(({ p1, p2, key }) => (
            <LineLengthLabel
              key={key}
              p1={p1}
              p2={p2}
          ...
          {modelLines.map((line, idx) => {
            ...
            // 计算延伸端点
            const dir = new THREE.Vector3().subVectors(p2, p1).normalize();
            const extP1 = line.extendBefore > 0
              ? p1.clone().sub(dir.clone().multiplyScalar(line.extendBefore))
              : null;
            const extP2 = line.extendAfter > 0
